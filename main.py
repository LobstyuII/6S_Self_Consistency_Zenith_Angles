                ...
                for band_id in config.BANDS.keys():
                    ...
                    if data_file.exists() and not args.debug and not args.force_repair:
                        ...
                        try:
                            ...
                            # 验证数据列
                            if not validate_data_columns(df_result, band_id):
        ...
        if args.phase in ['all', 'analyze']:
            ...
            if args.band == 'all':
                ...
                try:
                    ...
                    analyzer_all.create_summary_figure(band_id='all', save_path=summary_file)  # 修改这里
                ...
                # 然后分析每个波段单独的数据
                for band_id in list(results.keys()):
                    try:
                        logger.info(f"分析波段 {band_id}...")

                        # 检查是否有误差数据
                        if 'error_absolute' not in results[band_id].columns:
                            logger.warning(f"波段 {band_id} 没有误差数据，跳过")
                            continue

                        # 检查数据量
                        if len(results[band_id]) < 10:
                            logger.warning(f"波段 {band_id} 数据量不足 ({len(results[band_id])})，跳过详细分析")
                            continue

                        # 创建分析器
                        band_results = {band_id: results[band_id]}
                        ...
                        try:
                            ...
                            with open(report_file, 'w', encoding='utf-8') as f:
                                ...
                                f.write(f"样本数: {len(results[band_id])}\n")
                                f.write(f"有效误差数据: {results[band_id]['error_absolute'].notna().sum()}\n")
                                if band_id in stats:
                        ...
                        try:
                            ...
                            analyzer.create_summary_figure(band_id=band_id, save_path=summary_file)  # 修改这里
                ...
                # 单个波段
                if 'error_absolute' in results[args.band].columns:
                    # 确保有误差数据
                    ...
                    if error_count == 0:
                        ...
                        try:
                            ...
                            analyzer.create_summary_figure(band_id=args.band, save_path=summary_file)  # 修改这里
        ...
        if args.phase in ['all', 'sensitivity']:
            ...
            if args.band == 'all':
                ...
                if 'error_absolute' in results[args.band].columns:
                    ...
                    # 检查是否有足够的误差数据
                    if band_data['error_absolute'].notna().sum() < 10:
                        logger.warning("误差数据不足，跳过敏感性分析")
                    else:
                        # 创建敏感性分析器
