...
class EnhancedMLModels:
    ...
    def save_training_artifacts(self):
        ...
        # 保存特征名称
        features_path = self.output_dir / "feature_names.pkl"
        ...
        for model_name, config in self.model_configs.items():
            model_configs_simple[model_name] = {
                'description': config['description'],
                'color': config['color']
            }

        config_path = self.output_dir / "model_configs.json"
        with open(config_path, 'w', encoding='utf-8') as f:
            json.dump(model_configs_simple, f, indent=2, ensure_ascii=False)

        print(f"✅ 训练关键信息已保存到: {self.output_dir}")
...
class SavedModelsAnalyzer:
    """用于分析已保存的模型"""

    def __init__(self, saved_dir: Path, config: ExperimentConfig = None, logger=None):
        self.saved_dir = Path(saved_dir)
        self.config = config or ExperimentConfig
        self.logger = logger or setup_logger('SavedModelsAnalyzer')

        # 验证目录结构
        if not self.saved_dir.exists():
            raise ValueError(f"保存的模型目录不存在: {self.saved_dir}")

        self.models_dir = self.saved_dir / "models"
        self.plots_dir = self.saved_dir / "plots"
        self.tables_dir = self.saved_dir / "tables"
        ...
        print(f"✅ 已从 {self.saved_dir} 加载 {len(self.models)} 个模型")
    ...
    def _load_model_configs(self) -> Dict[str, Dict]:
        """加载模型配置"""
        config_path = self.saved_dir / "model_configs.json"
        ...
        for model_name in self.models.keys():
            configs[model_name] = {
                'description': f'已加载的{model_name}模型',
                'color': '#1f77b4'  # 默认颜色
            }
        return configs

    def load_test_data(self, sample_fraction: float = 1.0) -> pd.DataFrame:
        """加载测试数据"""
        print("=" * 70)
        print("📂 加载测试数据...")
        ...
        data = pd.concat(all_data, ignore_index=True)
        print(f"✅ 测试数据加载完成，总样本数: {len(data):,}")
    ...
    def prepare_features(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if scaler_path.exists():
            ...
            print(f"   已加载并应用保存的标准化器")
        else:
            print(f"   警告: 未找到保存的标准化器，使用新的标准化器")
    ...
    def evaluate_models(self, X_test: pd.DataFrame, y_test: pd.Series) -> pd.DataFrame:
        ...
        for model_name, model in self.models.items():
            try:
                ...
                result_row = {
                    'Model': model_name,
                    'Description': self.model_configs.get(model_name, {}).get('description', f'{model_name}模型'),
                    **metrics
                }
                results.append(result_row)
    ...
    def _plot_model_comparison(self, results_df: pd.DataFrame):
        ...
        colors = []
        for m in results_df['Model']:
            if m in self.model_configs and 'color' in self.model_configs[m]:
                colors.append(self.model_configs[m]['color'])
            else:
                colors.append('#1f77b4')  # 默认颜色
        ...
        # 6. 标准差
        ax = axes[1, 2]
        ax.bar(results_df['Model'], results_df['Std_Residuals'], alpha=0.8)
        ax.set_xlabel('模型')
        ax.set_ylabel('残差标准差')
        ax.set_title('残差标准差比较')
        ax.set_xticklabels(results_df['Model'], rotation=45, ha='right')
        ax.grid(True, alpha=0.3)
    ...
    def generate_report(self, results_df: pd.DataFrame):
        ...
        # 保存JSON报告
        report_path = self.saved_dir / "analysis_summary.json"
        with open(report_path, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)

        # 生成文本报告
        text_report = self._generate_text_report(results_df)
        text_path = self.saved_dir / "analysis_report.txt"
        with open(text_path, 'w', encoding='utf-8') as f:
            f.write(text_report)
    ...
    def _generate_text_report(self, results_df: pd.DataFrame) -> str:
        ...
        report_lines = []
        report_lines.append("=" * 80)
        report_lines.append("已保存模型分析报告")
        report_lines.append("=" * 80)
        report_lines.append(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report_lines.append(f"模型目录: {self.saved_dir}")
        report_lines.append(f"分析模型数: {len(self.models)}")
        report_lines.append("")

        report_lines.append("模型性能排名:")
        report_lines.append("-" * 80)
        report_lines.append(f"{'排名':<4} {'模型':<15} {'RMSE':<12} {'R²':<10} {'MAE':<12}")
        report_lines.append("-" * 80)
...
def analyze_saved_models():
    ...
    try:
        ...
        # 创建分析器实例
        analyzer = SavedModelsAnalyzer(
            saved_dir=Path(args.saved_dir),
            config=config
        )

        # 加载测试数据
        test_data = analyzer.load_test_data(sample_fraction=args.sample)

        # 准备特征
        X_test, y_test = analyzer.prepare_features(test_data)

        # 评估模型
        results_df = analyzer.evaluate_models(X_test, y_test)

        # 生成可视化
        analyzer.generate_visualizations(X_test, y_test, results_df)
        ...
        print("\n📁 生成的文件:")
        print(f"  评估结果: {analyzer.tables_dir}/evaluation_results.csv")
        print(f"  图表文件: {analyzer.plots_dir}/")
        print(f"  分析报告: {analyzer.saved_dir}/analysis_summary.json")
        print("=" * 80)
...
def main():
    ...
    parser.add_argument('--fast_mode', action='store_true',
                        help='快速模式：只训练最快模型')
    parser.add_argument('--analyze_saved', action='store_true',
                        help='分析已保存的模型（需要--saved_dir参数）')
    parser.add_argument('--saved_dir', type=str, default=None,
                        help='已保存模型的目录路径（用于--analyze_saved模式）')

    args = parser.parse_args()

    # 如果指定了分析已保存模型，则调用分析函数
    if args.analyze_saved:
        if not args.saved_dir:
            print("❌ 请使用--saved_dir指定已保存模型的目录")
            return 1

        # 设置参数并调用分析函数
        import sys
        sys.argv = [sys.argv[0], '--saved_dir', args.saved_dir]
        if args.sample != 1.0:
            sys.argv.extend(['--sample', str(args.sample)])
        if args.data_dir:
            sys.argv.extend(['--data_dir', args.data_dir])

        return analyze_saved_models()
