...
class ExperimentConfig:
    ...
    BASE_DIR = Path("D:/6S_Self_Consistency_Zenith_Angles_v0.12")
    ...
    MONTE_CARLO_CONFIG = {
        ...
        'extreme_ratio': 0.3,          # 30%极端过采样
        'extreme_threshold': 85.0,     # 极端角度阈值（>85°）
        'total_samples_per_band': 100000,  # 每波段总样本数
    }

    # ========== 连续参数范围配置 (训练用) ==========
    PARAM_RANGES_CONTINUOUS = {
        'geometry': {
            'sza': {'min': 0.0, 'max': 89.0, 'distribution': 'uniform'},
            'vza': {'min': 0.0, 'max': 89.0, 'distribution': 'uniform'},
    ...
    # ========== LUT配置 (生成业务化LUT用，完全保留不动) ==========
    LUT_CONFIG = {
        'sza': {
            'regular': {'range': (0, 75), 'step': 5},
            'extreme': {'range': (75, 89), 'step': 1}
        },
        'vza': {
            'regular': {'range': (0, 75), 'step': 5},
            'extreme': {'range': (75, 89), 'step': 1}
        },
        'raa': {'range': (0, 180), 'step': 30},
    ...
    VALIDATION_GRID = {
        # 几何角度 - 覆盖全范围，极端部分更细致
        'sza': [0, 15, 30, 45, 60, 75, 80, 85, 86, 87, 88, 89],
        'vza': [0, 15, 30, 45, 60, 75, 80, 85, 86, 87, 88, 89],
        'raa': [0, 45, 90, 135, 180],
        # 物理参数保留关键变化
