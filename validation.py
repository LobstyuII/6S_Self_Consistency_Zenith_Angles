...
class ModelValidator:
    ...
    def plot_validation_results(self, data: pd.DataFrame, band_id: str = 'band3',
        ...
        ax4.set_xlabel('SZA区间 (度)')
        ax4.set_ylabel('平均残差')
        ax4.set_title('不同SZA的预测性能')
        ...
        ax4_twin.bar(x_pos, perf_by_sza[('error_absolute', 'count')],
                     alpha=0.3, color='gray', label='样本数')
        ax4_twin.set_ylabel('样本数')
        ax4_twin.legend(loc='upper right')

        plt.suptitle(f'模型验证结果 - {band_id} ({model_type})', fontsize=14)
        plt.tight_layout()
