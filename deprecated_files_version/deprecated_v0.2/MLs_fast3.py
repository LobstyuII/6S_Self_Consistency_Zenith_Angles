...
class AdvancedFeatureEngineer:
    ...
    def create_physically_meaningful_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 1. 三角函数特征（直接使用角度计算，避免弧度转换带来的共线性）
        features['cos_sza'] = np.cos(np.radians(df['sza']))
        features['cos_vza'] = np.cos(np.radians(df['vza']))
        features['sin_sza'] = np.sin(np.radians(df['sza']))
        features['sin_vza'] = np.sin(np.radians(df['vza']))
        features['cos_raa'] = np.cos(np.radians(df['raa']))

        # 2. 几何相互作用特征（选择最有物理意义的）
        # 散射角 - 重要的几何参数
        ...
        features['scattering_angle'] = np.degrees(
            np.arccos(-np.cos(sza_rad) * np.cos(vza_rad) +
                      np.sin(sza_rad) * np.sin(vza_rad) * np.cos(raa_rad))
        )

        # 3. 大气质量数（使用简单的近似，避免复杂计算）
        features['airmass_sza'] = 1.0 / features['cos_sza']
        features['airmass_vza'] = 1.0 / features['cos_vza']
        ...
        # 8. 关键交互特征（选择最重要的几个）
        features['aod_total_airmass'] = features['aod550'] * features['total_airmass']
        features['wavelength_cos_sza'] = features['wavelength'] * features['cos_sza']

        # 9. 反射率相关衍生特征（选择最有信息量的）
    ...
    @staticmethod
    def remove_redundant_features(features: pd.DataFrame, threshold: float = 0.95) -> pd.DataFrame:
        """
        移除冗余的特征（高度相关的特征）
        返回处理后的特征DataFrame
        """
        ...
        for i in range(len(corr_matrix.columns)):
            for j in range(i + 1, len(corr_matrix.columns)):
                if corr_matrix.iloc[i, j] > threshold:
                    ...
                    # 保留更简单的特征
                    if 'sin_' in col_j and 'cos_' not in col_i:
                        columns_to_drop.add(col_j)
                    elif 'sin_' in col_i and 'cos_' not in col_j:
                        columns_to_drop.add(col_i)
                    elif len(col_i) > len(col_j):
                        columns_to_drop.add(col_i)
                    else:
                        columns_to_drop.add(col_j)
        ...
        # 移除高度相关的特征
        if columns_to_drop:
            print(f"  移除 {len(columns_to_drop)} 个高度相关特征: {list(columns_to_drop)}")
        ...
        if constant_features:
            print(f"  移除 {len(constant_features)} 个常数特征: {constant_features}")
...
class CorrectedMachineLearningModelsV2:
    ...
    def __init__(self, config: ExperimentConfig = None, logger=None,
        ...
        self.logger.info(f"移除冗余特征: {remove_redundant}")
        ...
        configs = {
            ...
            'XGBoost': {
                ...
                'params': {
                    ...
                    'reg_lambda': [1, 1.5],
                    'random_state': [self.config.RANDOM_SEED],
                    'tree_method': ['hist'],  # 使用hist而不是gpu_hist以提高稳定性
    ...
    def prepare_features(self, data: pd.DataFrame, is_training: bool = True) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if self.use_feature_engineering:
            ...
            if self.remove_redundant:
                features = self.feature_engineer.remove_redundant_features(features, threshold=0.95)
            ...
            features['cos_raa'] = np.cos(np.radians(data['raa']))
            ...
            # 角度关系
            features['vza_sza_ratio'] = data['vza'] / (data['sza'] + 1e-6)
            features['vza_minus_sza'] = data['vza'] - data['sza']
    ...
    def _train_single_model(self, model_name: str, X_train: pd.DataFrame,
        ...
        # 对于树模型，设置一些默认参数以提高稳定性
        if model_name in ['XGBoost', 'LightGBM']:
            base_model = model_class(random_state=self.config.RANDOM_SEED, n_jobs=-1)
        else:
...
def main():
    ...
    parser.add_argument('--cv_folds', type=int, default=5,
                        help='交叉验证折数')
    parser.add_argument('--no_remove_redundant', action='store_true',
                        help='不移除冗余特征')
    ...
    print(f"移除冗余: {'禁用' if args.no_remove_redundant else '启用'}")
